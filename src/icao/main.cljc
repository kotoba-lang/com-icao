(ns icao.main
  "Kotodama WASM entrypoint for the Icao clean-room actor (L4) — Clojure port.
  ...
  facts (`icao.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"icao.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "icao-compat" :tier tier :entities entities} 200])
